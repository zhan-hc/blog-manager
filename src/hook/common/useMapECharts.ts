...
export default function useMapECharts(elRef: Ref<HTMLDivElement>, autoChartSize = false, animation: boolean = false, render: RenderType = RenderType.SVGRenderer, theme: ThemeType = ThemeType.Default) {
    // 渲染模式
    ...
    function watchEl() {
        // 给元素添加过渡
        if (animation && elRef.value) { elRef.value.style.transition = 'width 1s, height 1s' }
        const resizeObserver = new ResizeObserver((entries => resize()))
        elRef.value && resizeObserver.observe(elRef.value);
    }
